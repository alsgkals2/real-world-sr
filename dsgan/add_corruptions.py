...
parser.add_argument('--dataset', default='set5', type=str, help='selecting different datasets')
parser.add_argument('--resolution', default='lr', type=str, choices=['hr', 'lr'], help='choose whether to use HR or LR')
...
# input_dirs = [PATHS[opt.dataset]['jpeg'][opt.resolution]['train'],
...
input_dirs = [PATHS[opt.dataset]['jpeg'][opt.resolution]['valid']]
target_dirs = [PATHS[opt.dataset][opt.artifacts][opt.resolution]['valid']]
...
for index, input_dir in enumerate(input_dirs):
    ...
    files = [os.path.join(input_dir, x) for x in os.listdir(input_dir) if utils.is_image_file(x) and ('_LR' in x or '_HR' in x)]
