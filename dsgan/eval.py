...
parser.add_argument('--checkpoint', default='bs16_crop256', type=str, help='checkpoint model to start from')
...
if opt.checkpoint is not None:
    # _checkpoint = './checkpoints/temp' + '/300_G.pth'
    
    _checkpoint = './checkpoints/' + opt.checkpoint + '/epoch_300.tar'
...
if opt.saving:
    dir_index = 0
    while os.path.isdir(f'test_{opt.dataset}/' + opt.checkpoint + '/' + str(dir_index)):
        dir_index += 1
    summary_path = f'test_{opt.dataset}/' + opt.checkpoint + '/' + str(dir_index)
        
    log = Logger()
    os.makedirs(summary_path, exist_ok=True)
    log.open(summary_path + '/log_test.txt', mode='a')
    log.write('\n')
    ...
    with torch.no_grad():
        ...
        for index, (input_img, disc_img, target_img) in enumerate(val_bar):
            ...
            out_path = f'{summary_path}/target_fake_tex_disc_f-wav_t-wav_' + str(index)
            # list_save = {'input_img' : input_img.squeeze(), 'fake_img' : fake_img.squeeze(), 'target_img' : target_img.squeeze()}
        
            # [cv2.imwrite(f'{summary_path}/target_fake_crop_low_high_{index}_{key}.jpg', np.array((image.permute(1,2,0)*255).cpu().detach())) for key,image in list_save.items()]
        ...
        if len(val_loader) > 0:
            ...
            for index, image in enumerate(val_save_bar):
                ...
                out_path = f'{summary_path}/target_fake_tex_disc_f-wav_t-wav_' + str(index)
                cv2.imwrite(f'{summary_path}/target_fake_crop_low_high_' + str(index)+'.jpg', image)
                list_save = {'input_img' : input_img[1:], 'fake_img' : fake_img[1:], 'target_img' : target_img[1:]}
                # [cv2.imwrite(f'{summary_path}/target_fake_crop_low_high_{index}_{key}.jpg', image) for key,image in list_save.items()]
